...
render_lib = generator.lib( module = 'render', sources = [
  'backend.c', 'command.c', 'context.c', 'drawable.c', 'indexbuffer.c', 'program.c', 'render.c',
...
if not target.is_ios() and not target.is_android() and not target.is_tizen():
  ...
  if not configs == []:
    generator.bin( 'renderimport', [ 'main.c', 'glsl.c' ], 'renderimport', basepath = 'tools', implicit_deps = [ render_lib ], libs = [ 'render', 'window', 'resource', 'foundation' ], configs = configs, extralibs = gllibs, extraframeworks = glframeworks )
...
if toolchain.is_monolithic() or target.is_ios() or target.is_android() or target.is_tizen() or target.is_pnacl():
  ...
  if target.is_macosx() or target.is_ios() or target.is_android() or target.is_tizen():
    generator.app( module = '', sources = [ os.path.join( module, 'main.c' ) for module in test_cases ] + test_extrasources, binname = 'test-all', basepath = 'test', implicit_deps = [ render_lib ], libs = [ 'test', 'render', 'resource', 'window', 'foundation' ], resources = test_resources, includepaths = includepaths, extralibs = gllibs, extraframeworks = glframeworks )
  else:
    generator.bin( module = '', sources = [ os.path.join( module, 'main.c' ) for module in test_cases ] + test_extrasources, binname = 'test-all', basepath = 'test', implicit_deps = [ render_lib ], libs = [ 'test', 'render', 'resource', 'window', 'foundation' ], resources = test_resources, includepaths = includepaths, extralibs = gllibs, extraframeworks = glframeworks )
else:
  ...
  for test in test_cases:
    if target.is_macosx():
      ...
      generator.app( module = test, sources = [ 'main.c' ], binname = 'test-' + test, basepath = 'test', implicit_deps = [ render_lib ], libs = [ 'test', 'render', 'resource', 'window', 'foundation' ], resources = test_resources, includepaths = includepaths, extralibs = gllibs, extraframeworks = glframeworks )
    else:
      generator.bin( module = test, sources = [ 'main.c' ], binname = 'test-' + test, basepath = 'test', implicit_deps = [ render_lib ], libs = [ 'test', 'render', 'resource', 'window', 'foundation' ], includepaths = includepaths, extralibs = gllibs, extraframeworks = glframeworks )
