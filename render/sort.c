/* sort.c  -  Render library  -  Public Domain  -  2014 Mattias Jansson
 *
...
render_sort_merge(render_context_t** contexts, size_t contexts_count) {
	for (size_t i = 0, size = contexts_count; i < size; ++i)
		contexts[i]->order = radixsort_sort(contexts[i]->sort, contexts[i]->keys,
		                                    (size_t)atomic_load32(&contexts[i]->reserved, memory_order_acquire));
}
