/* texture.h  -  Render library  -  Public Domain  -  2014 Mattias Jansson / Rampant Pixels
 *
...
RENDER_API int
render_texture_compile(const uuid_t uuid, uint64_t platform, resource_source_t* source,
                       const uint256_t source_hash, const char* type, size_t type_length);

#else

#define render_texture_compile(uuid, platform, source, source_hash, type, type_length) (((void)sizeof(uuid)), ((void)sizeof(platform)), ((void)sizeof(source)), ((void)sizeof(source_hash)), ((void)sizeof(type)), ((void)sizeof(type_length)), -1)
