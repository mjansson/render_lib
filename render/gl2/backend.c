...
nvoglexpert_callback(unsigned int category, unsigned int id, unsigned int detail, int object, const char* msg) {
	log_warn(HASH_RENDER, STRING_CONST("nVidia OpenGL Expert error: Category 0x%08x, Message 0x%08x : %s"), category,
	         id, msg);
}
...
#error Not implemented
#endif
					atomic_store32(&backend_gl2->concurrent_used[icontext], 0, memory_order_release);
					break;
				}
			}
		}
...
_rb_gl2_enable_thread(render_backend_t* backend) {
	...
	if (!thread_context) {
		...
		if (atomic_cas32(&backend_gl2->context_used, 1, 0, memory_order_release, memory_order_acquire)) {
			...
			for (uint64_t icontext = 0; icontext < backend->concurrency; ++icontext) {
				if (atomic_cas32(&backend_gl2->concurrent_used[icontext], 1, 0, memory_order_release,
			...
			if (!thread_context) {
				log_warn(HASH_RENDER, WARNING_INVALID_VALUE,
				         STRING_CONST("Unable to enable thread for GL2 rendering, no free concurrent context"));
				return;
			}
		}
...
_rb_gl2_set_drawable(render_backend_t* backend, const render_drawable_t* drawable) {
	...
	if (backend_gl2->concurrency) {
		backend_gl2->concurrent_context = memory_allocate(HASH_RENDER, sizeof(void*) * backend_gl2->concurrency, 0,
		                                                  MEMORY_PERSISTENT | MEMORY_ZERO_INITIALIZED);
		backend_gl2->concurrent_buffer = memory_allocate(HASH_RENDER, sizeof(atomic32_t) * backend_gl2->concurrency, 0,
		                                                 MEMORY_PERSISTENT | MEMORY_ZERO_INITIALIZED);
	...
	switch (shader->shadertype) {
		...
		case SHADER_VERTEX:
			...
			if (!compiled) {
				...
				log_errorf(HASH_RENDER, ERROR_SYSTEM_CALL_FAIL,
				           STRING_CONST("Unable to compile shader: %.*s (handle %u)"), (int)log_length, log_buffer,
				           (unsigned int)handle);
