...
static bool _rb_gl2_construct( render_backend_t* backend )
{
	//TODO: Caps check
	//if( !... )
	//  return false;

	log_debug( HASH_RENDER, "Constructed GL2 render backend" );
	return true;
}


static void _rb_gl2_destruct( render_backend_t* backend )
{
	...
	log_debug( HASH_RENDER, "Destructed GL2 render backend" );
}


static bool _rb_gl2_set_drawable( render_backend_t* backend, render_drawable_t* drawable )
{
	...
	if( !backend_gl2->context )
	{
		log_error( HASH_RENDER, ERROR_UNSUPPORTED, "Unable to create OpenGL 2 context" );
		return false;
	}
...
static void _rb_gl2_enable_thread( render_backend_t* backend )
{
	render_backend_gl2_t* backend_gl2 = (render_backend_gl2_t*)backend;
	...
	if( !wglMakeCurrent( (HDC)backend->drawable->hdc, (HGLRC)thread_context ) )
		...
		log_debug( HASH_RENDER, "Enabled thread for GL2 rendering" );
...
static void _rb_gl2_disable_thread( render_backend_t* backend )
{
	render_backend_gl2_t* backend_gl2 = (render_backend_gl2_t*)backend;
	...
	if( thread_context )
	{
		...
		log_debug( HASH_RENDER, "Disabled thread for GL2 rendering" );
	}
	set_thread_gl2_context( 0 );
}


unsigned int* _rb_gl2_enumerate_adapters( render_backend_t* backend )
{
...
render_resolution_t* _rb_gl2_enumerate_modes( render_backend_t* backend, unsigned int adapter )
{
...
static void* _rb_gl2_allocate_buffer( render_backend_t* backend, render_buffer_t* buffer )
{
	return memory_allocate( HASH_RENDER, buffer->size * buffer->allocated, 16, MEMORY_PERSISTENT );
}


static void _rb_gl2_deallocate_buffer( render_backend_t* backend, render_buffer_t* buffer, bool sys, bool aux )
{
	if( sys )
...
static void _rb_gl2_upload_buffer( render_backend_t* backend, render_buffer_t* buffer )
{
	GLuint buffer_object = (GLuint)buffer->backend_data[0];
	if( !buffer_object )
	{
...
static void _rb_gl2_upload_shader( render_backend_t* backend, render_shader_t* shader, const void* buffer, unsigned int size )
{
...
static void _rb_gl2_deallocate_shader( render_backend_t* backend, render_shader_t* shader )
{
	/*if( shader->backend_data[0] )
		glDeleteObject( shader->backend_data[0] );
	shader->backend_data[0] = 0;*/
...
#if 0
static render_parameter_block_t* _rb_gl2_allocate_parameter_block( render_backend_t* backend, const render_parameter_t* parameters, unsigned int num )
{
...
static void _rb_gl2_bind_parameter_block( render_backend_t* backend, render_parameter_block_t* block, unsigned int index, const void* RESTRICT data )
{
}


static void _rb_gl2_deallocate_parameter_block( render_backend_t* backend, render_parameter_block_t* block )
{
	memory_deallocate( block );
}


static void _rb_gl2_upload_texture( render_backend_t* backend, render_texture_t* texture, unsigned int width, unsigned int height, unsigned int depth, unsigned int levels, const void* data )
{
}


static render_texture_t* _rb_gl2_allocate_texture( render_backend_t* backend, render_texture_type_t type, render_usage_t usage, pixelformat_t format, colorspace_t colorspace )
{
...
static void _rb_gl2_dispatch( render_backend_t* backend, render_context_t** contexts, unsigned int num_contexts )
{
	render_backend_gl2_t* backend_gl2 = (render_backend_gl2_t*)backend;
...
render_backend_t* render_backend_gl2_allocate()
{
	if( !_rb_gl_check_context( 2, 0 ) )
		return 0;

	log_debug( HASH_RENDER, "Allocating GL2 render backend" );
	...
	render_backend_gl2_t* backend = memory_allocate( HASH_RENDER, sizeof( render_backend_gl2_t ), 0, MEMORY_PERSISTENT | MEMORY_ZERO_INITIALIZED );
...
render_backend_t* render_gl2_allocate()
{
	return 0;
}
