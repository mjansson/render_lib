...
_rb_gles2_ios_create_egl_context(void) {
	...
	if (!context) {
		log_warn(HASH_RENDER, WARNING_UNSUPPORTED, STRING_CONST("Unable to allocate GLES2 EAGL context"));
		return 0;
	}
	...
	log_debugf(HASH_RENDER, STRING_CONST("Initialized GLES2 EAGL context %" PRIfixPTR " version %d.%d"),
	           context, major, minor);
...
_rb_gles2_ios_render_buffer_storage_from_drawable(const void* context, const void* drawable,
	...
	if (![eagl_context renderbufferStorage:GL_RENDERBUFFER fromDrawable:layer]) {
		log_errorf(HASH_RENDER, ERROR_OUT_OF_MEMORY,
		           STRING_CONST("Unable to allocate render buffer storage"));
		return false;
	}
	...
	log_debugf(HASH_RENDER, STRING_CONST("Context 0x%" PRIfixPTR ", drawable 0x%" PRIfixPTR
	                                     ", colorbuffer %d storage allocated for drawable size %dx%d"),
	           context, drawable, colorbuffer, backing_width, backing_height);
...
_rb_gles2_ios_present_render_buffer(const void* context) {
	...
	if (![eagl_context presentRenderbuffer:GL_RENDERBUFFER])
		log_warnf(HASH_RENDER, WARNING_SUSPICIOUS, STRING_CONST("Failed presentRenderbuffer"));
}
