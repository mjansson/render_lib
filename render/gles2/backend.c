...
RENDER_EXTERN bool
_rb_gles2_ios_render_buffer_storage_from_drawable(const void* context, const void* drawable, unsigned int framebuffer,
...
_rb_gles2_check_error(const char* message, size_t length) {
	...
	if (err != GL_NONE) {
		...
		FOUNDATION_ASSERT_FAILFORMAT("OpenGLES2 error: %.*s: %s", STRING_FORMAT(message), _rb_gles2_error_message(err));
		return true;
	}
	return false;
}
...
_rb_gles2_log_error(const char* message, size_t length) {
	if (!_rb_gles2_check_error(message, length)) {
		log_errorf(HASH_RENDER, ERROR_SYSTEM_CALL_FAIL, STRING_CONST("%.*s: No GL error"), STRING_FORMAT(message));
		return false;
	}
	return true;
}
...
static size_t
_rb_gles2_enumerate_modes(render_backend_t* backend, unsigned int adapter, render_resolution_t* store,
                          size_t capacity) {
	FOUNDATION_UNUSED(backend);
	...
	render_resolution_t mode = {
	    0, _rb_gles2_ios_screen_width(), _rb_gles2_ios_screen_height(), PIXELFORMAT_R8G8B8X8, COLORSPACE_LINEAR, 60};
#else
...
_rb_gles2_enable_thread(render_backend_t* backend) {
	...
	if (!eglMakeCurrent(backend_gles2->display, backend_gles2->surface, backend_gles2->surface, backend_gles2->context))
		log_errorf(HASH_RENDER, ERROR_SYSTEM_CALL_FAIL, STRING_CONST("Unable to make context current on thread: %s"),
...
_rb_gles2_set_drawable(render_backend_t* backend, render_drawable_t* drawable) {
	...
	log_infof(HASH_RENDER, STRING_CONST("Initialized complete framebuffer object with dimensions %dx%d"), width,
	          height);
...
#define MAX_MATCHING_CONFIGS 64
	...
	log_debugf(HASH_RENDER, STRING_CONST("Window setBuffersGeometry( %d, %d ): %d"), drawable_width, drawable_height,
	           ret);

	backend_gles2->surface =
	    eglCreateWindowSurface(drawable->display, backend_gles2->config, (EGLNativeWindowType)drawable->native, 0);
	...
	if (success < 0) {
		log_errorf(HASH_RENDER, ERROR_SYSTEM_CALL_FAIL, STRING_CONST("Unable to get screen dimensions: %d"), success);
		return false;
	}
	...
	dispman_element =
	    vc_dispmanx_element_add(dispman_update, dispman_display, 0 /*layer*/, &dst_rect, 0 /*src*/, &src_rect,
	...
	backend_gles2->surface =
	    eglCreateWindowSurface(drawable->display, backend_gles2->config, (EGLNativeWindowType)native_window, 0);
	if (!backend_gles2->surface) {
		log_errorf(HASH_RENDER, ERROR_SYSTEM_CALL_FAIL, STRING_CONST("Unable to create EGL surface: %s"),
		           eglGetErrorMessage(eglGetError()));
		return false;
	...
	log_debugf(HASH_RENDER, STRING_CONST("Window initialized for EGL rendering: 0x%" PRIfixPTR " dimensions %dx%d"),
	           drawable->native, backend_gles2->framebuffer_width, backend_gles2->framebuffer_height);

	log_debugf(HASH_RENDER, STRING_CONST("Vendor     : %s"), eglQueryString(drawable->display, EGL_VENDOR));
	log_debugf(HASH_RENDER, STRING_CONST("Version    : %s"), eglQueryString(drawable->display, EGL_VERSION));
	log_debugf(HASH_RENDER, STRING_CONST("Extensions : %s"), eglQueryString(drawable->display, EGL_EXTENSIONS));
	log_debugf(HASH_RENDER, STRING_CONST("Client APIs: %s"), eglQueryString(drawable->display, EGL_CLIENT_APIS));

	//#if !FOUNDATION_BUILD_RTM
	int attributes[33] = {EGL_BUFFER_SIZE,
	                      EGL_ALPHA_SIZE,
	                      EGL_BLUE_SIZE,
	                      EGL_GREEN_SIZE,
	                      EGL_RED_SIZE,
	                      ...
	                      EGL_RENDERABLE_TYPE,
	                      EGL_CONFORMANT};
	const char* attribnames[33] = {"EGL_BUFFER_SIZE",
	                               "EGL_ALPHA_SIZE",
	                               "EGL_BLUE_SIZE",
	                               "EGL_GREEN_SIZE",
	                               "EGL_RED_SIZE",
			...
			switch (command->type) {
				...
				case RENDERCOMMAND_VIEWPORT: {
					...
					/*	case RENDERCOMMAND_RENDER:
					    {
					        render_vertexbuffer_t* vertexbuffer = pool_lookup( _global_pool_renderbuffer,
					command->data.render.vertexbuffer ); render_indexbuffer_t* indexbuffer  = pool_lookup(
					_global_pool_renderbuffer, command->data.render.indexbuffer ); render_vertexshader_t* vertexshader =
					pool_lookup( _global_pool_shader, command->data.render.vertexshader ); render_pixelshader_t*
					pixelshader  = pool_lookup( _global_pool_shader, command->data.render.pixelshader );
					        render_parameter_block_t* block = pool_lookup( _global_pool_parameterblock,
					command->data.render.parameterblock );
					        ...
					        //Bind vertex attributes
					        {
					            ...
					            for( unsigned int attrib = 0; attrib < VERTEXATTRIBUTE_NUMATTRIBUTES; ++attrib )
					            {
					                ...
					                {
					                    glVertexAttribPointer( attrib, _rb_gles2_vertex_format_size[format],
					_rb_gles2_vertex_format_type[format], _rb_gles2_vertex_format_norm[format], vertexbuffer->size,
					(const void*)(uintptr_t)decl->attribute[attrib].offset ); glEnableVertexAttribArray( attrib );
					                }
					                else
					                {
					        ...
					        render_program_gles2_t* program = _rb_gles2_program_map( (unsigned
					int)vertexshader->backend_data[0], (unsigned int)pixelshader->backend_data[0] ); glUseProgram(
					program->program );

					        // Bind the parameter blocks
					        render_parameter_info_t* param_info = block->info;
					        hash_t* param_name = pointer_offset( block, sizeof( render_parameter_block_t ) + ( sizeof(
					render_parameter_info_t ) * block->num ) ); for( unsigned int ip = 0; ip < block->num; ++ip,
					++param_info, ++param_name )
					        {
					            if( param_info->type == RENDERPARAMETER_TEXTURE )
					            {
					                //TODO: Dynamic use of texture units, reusing unit that already have correct texture
					bound, and least-recently-used evicting old bindings to free a new unit glActiveTexture( GL_TEXTURE0
					+ param_info->unit );

					                object_t object = *(object_t*)pointer_offset( block, param_info->offset );
					                render_texture_gles2_t* texture = object ? pool_lookup( _global_pool_texture, object
					) : 0; NEO_ASSERT_MSGFORMAT( !object || texture, "Parameter block using old/invalid texture 0x%llx",
					object );
					                ...
					                for( unsigned int iu = 0; iu < program->num_uniforms; ++iu )
					                {
					                    if( program->uniforms[iu].name == *param_name )
					                    {
					                        if( param_info->type == RENDERPARAMETER_FLOAT4 )
					                            glUniform4fv( program->uniforms[iu].location, param_info->dim, (const
					GLfloat*)pointer_offset( block, param_info->offset ) ); else if( param_info->type ==
					RENDERPARAMETER_INT4 ) glUniform4iv( program->uniforms[iu].location, param_info->dim, (const
					GLint*)pointer_offset( block, param_info->offset ) ); else if( param_info->type ==
					RENDERPARAMETER_MATRIX ) glUniformMatrix4fv( program->uniforms[iu].location, param_info->dim,
					GL_FALSE, (const GLfloat*)pointer_offset( block, param_info->offset ) ); break;
					                    }
					                }
					            }
					        }
					...
					0xFFFFFFFF );
					        ...
					        if( command->data.render.blend_state )
					        {
					            ...
					            unsigned int dest_color = (unsigned int)( ( command->data.render.blend_state >> 4ULL ) &
					0xFULL );
					        ...
					        unsigned int pnum = _rb_gles2_primitive_mult[primitive] * num +
					_rb_gles2_primitive_add[primitive];

					        glDrawElements( _rb_gles2_primitive_type[primitive], pnum, ( indexbuffer->size == 2 ) ?
...
static void
_rb_gles2_deallocate_buffer(render_backend_t* backend, render_buffer_t* buffer, bool sys, bool aux) {
	if (sys)
...
static bool
_rb_gles2_upload_shader(render_backend_t* backend, render_shader_t* shader, const void* buffer, size_t size) {
	bool ret = false;
	switch (shader->shadertype) {
		case SHADER_VERTEX:
			...
			if (!compiled) {
				...
				log_errorf(HASH_RENDER, ERROR_SYSTEM_CALL_FAIL, STRING_CONST("Unable to compile vertex shader: %.*s"),
				           (int)log_length, log);
		...
		case SHADER_PIXEL:
			...
			if (size < 16) {
				log_errorf(HASH_RENDER, ERROR_INVALID_VALUE, STRING_CONST("Invalid pixel shader code: %.*s"), (int)size,
				           (const char*)buffer);
				return;
			}
			// TODO: Binary formats
			...
			if (!compiled) {
				...
				GLint log_length = 0;
				glGetShaderInfoLog(shader_object, buffer_size, &log_length, log);
				log_errorf(HASH_RENDER, ERROR_SYSTEM_CALL_FAIL, STRING_CONST("Unable to compile pixel shader: %.*s"),
				           (int)log_length, log);
				memory_deallocate(log);
				log_debugf(HASH_RENDER, STRING_CONST("Shader source:\n%.*s"), (int)size, (const char*)buffer);
				break;
...
_rb_gles2_program_unmap(GLuint vertex_shader, GLuint pixel_shader) {
	...
	{
	    if( ( _rb_gles2_programs[ip].vertex_shader == vertex_shader ) || ( _rb_gles2_programs[ip].pixel_shader ==
	pixel_shader ) )
	    {
...
static void
_rb_gles2_link_buffer(render_backend_t* backend, render_buffer_t* buffer, render_program_t* program) {
	FOUNDATION_UNUSED(backend);
...
static render_backend_vtable_t _render_backend_vtable_gles2 = {.construct = _rb_gles2_construct,
...
render_backend_gles2_allocate(void) {
	render_backend_t* backend =
	    memory_allocate(HASH_RENDER, sizeof(render_backend_gles2_t), 0, MEMORY_PERSISTENT | MEMORY_ZERO_INITIALIZED);
