/* main.c  -  Render library shader view  -  Public Domain  -  2014 Mattias Jansson / Rampant Pixels
 *
...
main_initialize(void) {
	...
	application.name = string_const(STRING_CONST("shaderview"));
	application.short_name = string_const(STRING_CONST("shaderview"));
...
main_run(void* main_arg) {
	int result = 0;
	shaderview_input_t input = shaderview_parse_command_line(environment_command_line());

	FOUNDATION_UNUSED(main_arg);

	for (size_t cfgfile = 0, fsize = array_size(input.config_files); cfgfile < fsize; ++cfgfile)
		sjson_parse_path(STRING_ARGS(input.config_files[cfgfile]), shaderview_parse_config);
