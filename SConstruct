...
opts.AddVariables(
	...
	( 'vectorpath',                 'Path to vector library', '' ),
)
...
VariantDir( 'build/scons/%s/render' % env['buildpath'] , 'render', duplicate=0 )
SConscript( 'build/scons/%s/render/SConscript' % env['buildpath']  )
